...
def send_suggestion_email(sender_name: str, sender_email: str, message: str) -> None:
    """Reenvía una sugerencia al correo del administrador."""
    admin_email = settings.SMTP_USER or settings.SMTP_FROM

    if not settings.SMTP_HOST or not settings.SMTP_USER or not settings.SMTP_PASSWORD:
        print(f"\n[DIRBOOK - DEV] Sugerencia de {sender_name} <{sender_email}>:")
        print(f"  {message}\n")
        return
    ...
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Nueva sugerencia de {sender_name} — Dirbook"
    msg["From"]    = f"Dirbook <{from_addr}>"
    msg["To"]      = admin_email
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 20px;">
    ...
    try:
        ...
        print(f"[DIRBOOK] Sugerencia de {sender_email} enviada a {admin_email}")
    except Exception as exc:
        print(f"[DIRBOOK] Error al enviar sugerencia: {exc}")
