...
const TYPE_OPTIONS: { value: PostType; label: string; desc: string }[] = [
  { value: "post",     label: "Publicación",  desc: "Novedad, logro o anuncio general" },
  { value: "event",    label: "Evento",       desc: "Charla, taller o evento con fechas" },
  { value: "discount", label: "Promoción",    desc: "Descuento o tarifa especial por tiempo" },
];

export default function NewProfessionalPostPage() {
  const { id: profId } = useParams<{ id: string }>();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) { const r = await uploadImage(imageFile); image_url = r.url; }
      ...
      router.push(`/professionals/${profId}`);
    } catch (err) {
    ...
    <div className="mx-auto max-w-xl px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {TYPE_OPTIONS.map(({ value, label, desc }) => (
            <button key={value} type="button" onClick={() => setPostType(value)}
              className={`rounded-xl border p-3 text-left transition-colors ${
                postType === value ? "border-primary bg-primary/5" : "border-muted hover:border-muted-foreground/40"
              }`}>
        ...
        <div className="space-y-1">
          ...
          <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Título" />
        </div>

        <div className="space-y-1">
          <Label htmlFor="content">Descripción *</Label>
          <Textarea id="content" value={content} onChange={(e) => setContent(e.target.value)} placeholder="¿Qué quieres publicar?" rows={4} required />
        </div>
